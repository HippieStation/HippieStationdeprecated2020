...
/client/proc/cmd_show_at_markers()
	...
	if(count)
		to_chat(usr, "[count] AT markers removed.", confidental = TRUE)
	else
		for(var/t in GLOB.active_turfs_startlist)
			new /obj/effect/abstract/marker/at(t)
			count++
		to_chat(usr, "[count] AT markers placed.", confidental = TRUE)
...
/client/proc/count_objects_on_z_level()
	...
	to_chat(world, "There are [count] objects of type [type_path] on z-level [num_level]", confidental = TRUE)
...
/client/proc/count_objects_all()
	...
	to_chat(world, "There are [count] objects of type [type_path] in the game world", confidental = TRUE)
