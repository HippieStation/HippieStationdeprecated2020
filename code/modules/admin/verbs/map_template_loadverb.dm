/client/proc/map_template_load()
	...
	if(alert(src,"Confirm location.","Template Confirm","Yes","No") == "Yes")
		if(template.load(T, centered = TRUE))
			...
			to_chat(src, "Failed to place map", confidental = TRUE)
	images -= preview
...
/client/proc/map_template_upload()
	...
	if(copytext("[map]",-4) != ".dmm")
		to_chat(src, "<span class='warning'>Filename must end in '.dmm': [map]</span>", confidental = TRUE)
		return
	...
	switch(alert(src, "What kind of map is this?", "Map type", "Normal", "Shuttle", "Cancel"))
		...
		if("Shuttle")
			...
			return
	if(!M.cached_map)
		to_chat(src, "<span class='warning'>Map template '[map]' failed to parse properly.</span>", confidental = TRUE)
		return
	...
	if(report)
		...
		to_chat(src, "<span class='warning'>Map template '[map]' <a href='?src=[REF(report)];[HrefToken()];show=1'>failed validation</a>.</span>", confidental = TRUE)
		if(report.loadable)
	...
	to_chat(src, "<span class='notice'>Map template '[map]' ready to place ([M.width]x[M.height])</span>", confidental = TRUE)
