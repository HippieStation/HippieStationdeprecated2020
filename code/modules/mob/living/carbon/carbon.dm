...
/mob/living/carbon/throw_item(atom/target)
	...
	else if(!(I.flags_1 & (NODROP_1|ABSTRACT)))
		thrown_thing = I
		dropItemToGround(I)
...
/mob/living/carbon/show_inv(mob/user)
	...
	<HR>
	<BR><B>Head:</B> <A href='?src=[REF(src)];item=[SLOT_HEAD]'>				[(head && !(head.flags_1&ABSTRACT)) 			? head 		: "Nothing"]</A>
	<BR><B>Mask:</B> <A href='?src=[REF(src)];item=[SLOT_WEAR_MASK]'>		[(wear_mask && !(wear_mask.flags_1&ABSTRACT))	? wear_mask	: "Nothing"]</A>
	<BR><B>Neck:</B> <A href='?src=[REF(src)];item=[SLOT_NECK]'>		[(wear_neck && !(wear_neck.flags_1&ABSTRACT))	? wear_neck	: "Nothing"]</A>"}

	for(var/i in 1 to held_items.len)
		var/obj/item/I = get_item_for_held_index(i)
		dat += "<BR><B>[get_held_index_name(i)]:</B></td><td><A href='?src=[REF(src)];item=[SLOT_HANDS];hand_index=[i]'>[(I && !(I.flags_1 & ABSTRACT)) ? I : "Nothing"]</a>"
...
/mob/living/carbon/proc/accident(obj/item/I)
	if(!I || (I.flags_1 & (NODROP_1|ABSTRACT)))
		return
