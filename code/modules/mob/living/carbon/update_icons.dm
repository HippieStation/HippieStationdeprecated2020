...
/mob/living/carbon/update_inv_head()
	...
	if(head)
		...
		var/mutable_appearance/hm = head.build_worn_icon(default_layer = HEAD_LAYER, default_icon_file = 'icons/mob/clothing/head.dmi')
		...
		if (istype(head, /obj/item/clothing/head))
			...
			if (LAZYLEN(H.stacked_hats) > 0)
				...
				for (var/obj/item/clothing/head/J in H.stacked_hats)
					if (istype(J))
						var/mutable_appearance/new_hat = J.build_worn_icon(default_layer = HEAD_LAYER, default_icon_file = 'icons/mob/clothing/head.dmi')
						new_hat.pixel_y += 3 * I
