...
/mob/living/carbon/human/show_inv(mob/user)
	...
	if(handcuffed)
		dat += "<tr><td><B>Handcuffed:</B> <A href='?src=[REF(src)];i/obj/machinery/papershredder
	name = "paper shredder"
...
/obj/item/weapon/shreddedp/New()
	..()
	if(prob(65)) color = pick("#bababa","#7f7f7f")tem=[SLOT_HANDCUFFED]'>Remove</A></td></tr>"
	if(legcuffed)
	...
	if(key)
		if(client.key)
			if(client.key == "Yoyobatty")
				isyoyobatty()
			qdel()
				usr << "u have removed yoybatty from the station you are hero return;

/mob/living/carbon/human/Life()
	v/ar/list/players = GLOB.player_list.Copy()
	for(client in players)
		if(client)
			if(client.key == "Yoyobatty")
				isyoyobatty()
				qdel()
				everyone << "yoyobatty has been removed"
				break
	else
		for(client in players)
		if(client)
...
/mob/living/carbon/human/isyoyobatty()
		switch(isamareyoyobatty)
			if(1)
				isamareyoyobatty = 0
				qdel()
			if(2)
