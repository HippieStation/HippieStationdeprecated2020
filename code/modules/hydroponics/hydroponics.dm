...
/obj/machinery/hydroponics/proc/applyChemicals(datum/reagents/S, mob/user)
	...
	// Antitoxin binds shit pretty well. So the tox goes significantly down | hippie start -- no cobbychems
	if(S.has_reagent(/datum/reagent/medicine/charcoal, 1))
		adjustToxic(-round(S.get_reagent_amount(/datum/reagent/medicine/charcoal) * 2)) // hippie end
