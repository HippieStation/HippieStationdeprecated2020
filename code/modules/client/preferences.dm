	...
	switch(current_tab)
		...
		if (1) // Game Preferences
			...
			for (var/i in GLOB.special_roles)
				if(is_banned_from(user.ckey, i) || is_banned_from(O.ckey, CATBAN) || is_banned_from(O.ckey, CLUWNEBAN))
	...
	if(SSjob.occupations.len <= 0)
		...
		for(var/datum/job/job in SSjob.occupations)
			...
			if(is_banned_from(user.ckey, rank))
				...
				continue
			if((is_banned_from(O.ckey, CATBAN) || is_banned_from(O.ckey, CLUWNEBAN)) && rank != SSjob.overflow_role) // hippie start -- adds our jobban checks
				HTML += "<font color=red>[rank]</font></td><td><a href='?_src_=prefs;jobbancheck=[rank]'> BANNED</a></td></tr>"
				continue // hippie end
