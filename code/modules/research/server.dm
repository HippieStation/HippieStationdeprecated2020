/obj/machinery/rnd/server
	...
	var/server_id = 0
	var/base_mining_income = 80 //HIPPIE CODE- Changed from 2
	var/heat_gen = 100
...
/obj/machinery/rnd/server/RefreshParts()
	var/tot_rating = 0
	for(var/obj/item/stock_parts/SP in component_parts) //HIPPIECODE - changed src to component_parts
		tot_rating += SP.rating
	heat_gen /= max(1, tot_rating)
	part_bonus = 1 + max(((tot_rating-1)*part_value), 0) //HIPPIE CODE - added to take in account parts
...
/obj/machinery/rnd/server/proc/mine()
	. = base_mining_income
	var/penalty = max((get_env_temp() - temp_tolerance), 0) * temp_penalty_coefficient //HIPPIE CODE - Change temp_tolerance_low), 0) / temp_penalty_coefficient to temp_tolerance), 0) * temp_penalty_coefficient
	. = max(. - penalty, 0)
