...
/obj/machinery/door/window/open(forced=FALSE)
	...
	if(!operating) //in case of emag
		operating = TRUE
	do_animate("opening")
	playsound(src, 'hippiestation/sound/machines/windowdoor.ogg', 100, TRUE) // hippie -- changes the sound to our hippiestation/ variant. also not going to mirror this because then carbon will ask me to fix the sleep
	icon_state ="[base_state]open"
	sleep(10)
...
/obj/machinery/door/window/close(forced=FALSE)
	...
	operating = TRUE
	do_animate("closing")
	playsound(src, 'hippiestation/sound/machines/windowdoor.ogg', 100, TRUE) // hippie -- changes the sound to our hippiestation variant
	icon_state = base_state
