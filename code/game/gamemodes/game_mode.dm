...
/datum/game_mode/proc/convert_roundtype()
	...
	for(var/mob/living/carbon/human/H in living_crew)
		if(H.client && H.client.prefs.allow_midround_antag && !is_centcom_level(H.z))
			if(!IsCatbanned(H.ckey) && !is_banned_from(H.ckey, CLUWNEBAN) && !HAS_TRAIT(H, TRAIT_MINDSHIELD)) // hippie -- adds our jobban checks, cockblocks mindshielded people
				antag_candidates += H
...
/datum/game_mode/proc/get_players_for_role(role)
	...
	for(var/mob/dead/new_player/player in GLOB.player_list)
		if(player.client && player.ready == PLAYER_READY_TO_PLAY && player.check_preferences())
			if(!IsCatbanned(player.ckey) && !is_banned_from(player.ckey, CLUWNEBAN) && !is_banned_from(player.ckey, CRABBAN)) // hippie -- adds our jobban checks
				players += player
