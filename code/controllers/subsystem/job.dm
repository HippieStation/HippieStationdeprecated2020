...
/datum/controller/subsystem/job/proc/FindOccupationCandidates(datum/job/job, level, flag)
	...
	for(var/mob/dead/new_player/player in unassigned)
		if(is_banned_from(player.ckey, job.title) || QDELETED(player))
			...
			continue
		if((is_banned_from(player.ckey, CLUWNEBAN) || IsCatbanned(player.ckey) || is_banned_from(player.ckey, CRABBAN)) && job.title != overflow_role) // hippie start -- fixes catbans
...
/datum/controller/subsystem/job/proc/GiveRandomJob(mob/dead/new_player/player)
	...
	for(var/datum/job/job in shuffle(occupations))
		...
		if(is_banned_from(player.ckey, CLUWNEBAN) || IsCatbanned(player.ckey) || is_banned_from(player.ckey, CRABBAN)) // hippie start -- fixes catbans
