...
/obj/item/integrated_circuit/power/weapon
	name = "weapon cell charger"
	desc = "This can wirelessly transmit electricity from an assembly's battery towards the cell of a weapon. A portable version of the weapon charger, it was declared too dangerous to be put in the hands of normal crewmembers and thus declared contraband."
	icon_state = "power_transmitter"
	extended_desc = "This circuit transmits 5 kJ of electricity every time the activator pin is pulsed. The input pin must be \
	a reference to an electricity gun to send electricity to. \
	The power is sourced from the assembly's power cell. If the target is outside of the assembly, \
	some power is lost due to ineffiency."
	w_class = WEIGHT_CLASS_SMALL
	complexity = 81
	inputs = list("target" = IC_PINTYPE_REF)
	...
	activators = list("transmit" = IC_PINTYPE_PULSE_IN, "on transmit" = IC_PINTYPE_PULSE_OUT)
	spawn_flags = IC_SPAWN_EMAG|IC_SPAWN_RESEARCH
	power_draw_per_use = 500 // Inefficency has to come from somewhere.
	var/amount_to_move = 5000

/obj/item/integrated_circuit/power/weapon/do_work()
	...
	if(cell)
		...
		if(A.Adjacent(B))
			if(AM.loc != assembly)
				transfer_amount *= 0.8 // Losses due to distance.
			var/list/U=A.GetAllContents(/obj/item/integrated_circuit/power/weapon)
			transfer_amount *= 1 / U.len
			set_pin_data(IC_OUTPUT, 1, cell.charge)
			set_pin_data(IC_OUTPUT, 2, cell.maxcharge)
			set_pin_data(IC_OUTPUT, 3, cell.percent())
