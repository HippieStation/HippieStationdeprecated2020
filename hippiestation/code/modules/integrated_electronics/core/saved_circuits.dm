...
// "requires_upgrades", "unsupported_circuit", "requires_emag", "metal_cost", "complexity", "max_complexity", "used_space", "max_space",
/datum/controller/subsystem/processing/circuit/proc/validate_electronic_assembly(program)
	...
	for(var/C in blocks["components"])
		...
		// Check if the assembly requires the printer to be emagged
		if(!(component.spawn_flags & IC_SPAWN_EMAG))
			blocks["requires_emag"] = TRUE
