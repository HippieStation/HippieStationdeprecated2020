...
/turf/open/pool/attack_hand(mob/user)
	if(!user.stat && !user.lying && Adjacent(user) && user.swimming && filled && src.next_splash < world.time) //not drained, user alive and close, and user in water.
		if(user.x == src.x && user.y == src.y)
			return
		else
			playsound(src, 'hippiestation/sound/effects/watersplash.ogg', 8, 1, 1)
			src.splashed = world.time + 25
