			...
			if(!applicant.stat)
				if(applicant.mind)
					if(!applicant.mind.special_role)
						if(!is_banned_from(applicant.ckey, ROLE_SHADOWLING) && !is_banned_from(applicant.ckey, ROLE_SYNDICATE) && !is_banned_from(applicant.ckey, CLUWNEBAN) && !IsCatbanned(applicant.ckey))
...
/datum/admins/proc/makeVampire()
	...
	for(var/mob/living/carbon/human/applicant in GLOB.player_list)
		if((ROLE_VAMPIRE in applicant.client.prefs.be_special) && !applicant.stat && applicant.mind && !applicant.mind.special_role)
			if(!is_banned_from(applicant.ckey, ROLE_VAMPIRE) && !is_banned_from(applicant.ckey, ROLE_SYNDICATE) && !is_banned_from(applicant.ckey, CLUWNEBAN) && !IsCatbanned(applicant.ckey))
