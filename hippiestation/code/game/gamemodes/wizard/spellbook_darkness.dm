...
/obj/item/bookofdarkness/attack_self(mob/living/user)
	...
	to_chat(user, "<font color=purple><b>Soulflare</b></font> deals 15 burn, brute and toxins damage to the target, putting them asleep for three seconds and if they are already in critical condition, they are instantly killed and the spell is refunded. This also applies to corpses.")
	...
	to_chat(user, "<font color=purple><b>Soulsplit</b></font> let's you become incorporeal for 3.5 seconds, allowing you to phase through objects and walk at very high speeds. However, it cannot be cast if you are below 80 health. In addition, you are still vulnerable to damage and other attacks in this state, nor will it remove any stuns.")
