		...
		if("restart")
			if(CONFIG_GET(flag/allow_vote_restart) || usr.client.holder)
				if(min_restart_time < world.time)
					...
					to_chat(world, "<span style='boldannounce'>Restart can only initiate after 60 minutes.</span>")
					message_admins("A restart vote cannot start until 60 minutes into the round.")
		if("shuttlecall")
			if(CONFIG_GET(flag/allow_vote_shuttlecall) || usr.client.holder)
				if(min_shuttle_time < world.time)
					...
					to_chat(world, "<span style='boldannounce'>Shuttle call can only initiate after 30 minutes.</span>")
					message_admins("A shuttle call vote cannot start until 30 minutes into the round.")
		if("gamemode")
